<h1 align="center">
  ...
  <br>
  OpenHarness — VS Code Extension Fork
</h1>

<p align="center">
  <a href="https://github.com/HKUDS/OpenHarness"><img src="https://img.shields.io/badge/upstream-HKUDS%2FOpenHarness-blue?style=for-the-badge" alt="Upstream"></a>
  <a href="https://github.com/victor1338/openharness-vscode"><img src="https://img.shields.io/badge/VS_Code_Extension-victor1338-purple?style=for-the-badge" alt="Extension"></a>
...
This is a **fork** of [HKUDS/OpenHarness](https://github.com/HKUDS/OpenHarness), specialized as the **Python backend** for the [OpenHarness VS Code Extension](https://github.com/victor1338/openharness-vscode).

**OpenHarness** is an open-source, lightweight AI agent infrastructure providing tool-use, skills, memory, and multi-agent coordination. This fork adds enhancements tailored for the VS Code integration while staying in sync with the upstream project.

## Why a fork?

The VS Code extension requires certain adjustments to the OpenHarness backend that are specific to the extension's needs:

- **Thinking/reasoning token streaming** — Streams the model's internal reasoning process (e.g. from DeepSeek-Reasoner, o1) to the VS Code chat UI in real time
- **Backend event protocol extensions** — Additional OHJSON protocol events (`thinking_delta`) for richer frontend rendering
- **VS Code-specific fixes** — Patches for edge cases encountered in the extension workflow (max-turns continuation, message sync, etc.)

These changes are maintained here so the extension can depend on a stable, tested backend.

## Relationship to upstream

| | This fork | Upstream |
|---|---|---|
| **Repo** | [victor1338/OpenHarness](https://github.com/victor1338/OpenHarness) | [HKUDS/OpenHarness](https://github.com/HKUDS/OpenHarness) |
| **Purpose** | Backend for [openharness-vscode](https://github.com/victor1338/openharness-vscode) | General-purpose agent infrastructure |
| **Sync** | Auto-syncs daily from upstream via GitHub Actions | — |
| **Conflicts** | Notified via GitHub issue + email | — |

> **All credit for the core OpenHarness project goes to [HKUDS](https://github.com/HKUDS) and the OpenHarness contributors.**

## How it's used

The VS Code extension installs this fork automatically:

1. **Local development** — If `OpenHarness/` is in the workspace, the extension runs `pip install -e .` (editable install)
2. **Standalone** — If no local source is found, the extension installs directly from this fork:
   ```
   pip install git+https://github.com/victor1338/OpenHarness.git
   ```

## Changes from upstream

All changes are tracked in commits on the `main` branch ahead of upstream. Key modifications:

| Area | Files | Change |
|------|-------|--------|
| Thinking stream | `api/client.py`, `api/openai_client.py` | Added `ApiThinkingDeltaEvent` — yields reasoning tokens from thinking models |
| Event pipeline | `engine/stream_events.py`, `engine/query.py` | Added `ThinkingDelta` stream event and forwarding |
| Backend protocol | `ui/backend_host.py`, `ui/protocol.py` | Emits `thinking_delta` OHJSON events to the extension |
| Max-turns fix | `engine/query_engine.py` | Syncs messages on `MaxTurnsExceeded` for `/continue` support |
| Auto-sync | `.github/workflows/sync-upstream.yml` | Daily sync from HKUDS/OpenHarness |

## Original project

For full documentation, features, installation, and usage of the core OpenHarness project, see:

- **[HKUDS/OpenHarness](https://github.com/HKUDS/OpenHarness)** — Original repository
- **[README (upstream)](README.upstream.md)** — Original README preserved in this repo

## License

MIT — same as the original [OpenHarness](https://github.com/HKUDS/OpenHarness/blob/main/LICENSE).

This fork is maintained by [victor1338](https://github.com/victor1338).
<h1 align="center">
  <img src="assets/logo.png" alt="OpenHarness" width="64" style="vertical-align: middle;">
  &nbsp;&nbsp;
